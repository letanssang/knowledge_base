# Mobile Security

Series chia sẻ kiến thức về Mobile Application Security từ góc nhìn của một developer: từ các tiêu chuẩn như OWASP MASVS/MASTG, cách bảo vệ dữ liệu, authentication, network communication đến reverse engineering và các kỹ thuật security testing trên Android/iOS.

Mục tiêu của series là hiểu tại sao một cơ chế bảo mật tồn tại, nó bảo vệ chúng ta khỏi điều gì và cách áp dụng nó vào ứng dụng thực tế, thay vì chỉ làm theo security checklist.

## Roadmap

| # | Article | Status |
| --- | --- | --- |
| 01 | [OWASP MASVS](./01-owasp-masvs/index.md) | Published |
| 02 | [OWASP MASWE](./02-owasp-maswe/index.md) | Published |
| 03 | [OWASP MASTG](./03-owasp-mastg/index.md) | Published |
| 04 | Secure Storage | Planned |
| 05 | Mobile Cryptography | Planned |
| 06 | Authentication | Planned |
| 07 | Network Security | Planned |
| 08 | Platform Security | Planned |
| 09 | Secure Coding | Planned |
| 10 | Reverse Engineering & Resilience | Planned |
| 11 | Mobile Privacy | Planned |
| 12 | Mobile Application Security Review — Putting Everything Together | Planned |
